...
toggle_tip   = texts.get("toggle_tooltip", "")
...
if page == "Overview":
    ...
    plot_line_chart(filtered_df, target_col, "None", container=tabs[0])
    hr_df  = apply_aggregation(filtered_df, COL_NAMES, target_col, "Hour", agg_funcs)
    plot_line_chart(hr_df, target_col, "Hour", container=tabs[1])
    day_df = apply_aggregation(filtered_df, COL_NAMES, target_col, "Day", agg_funcs)
    plot_line_chart(day_df, target_col, "Day", container=tabs[2])
